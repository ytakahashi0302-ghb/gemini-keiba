...
ここで、非現実的な大穴を推奨するバグを防ぐため、計算に用いるオッズには **最大50.0倍の上限（キャップ）** を設ける。

$$EV_i = P_i \times \min(O_i, 50.0)$$
...
* **高EV伏兵**: $EV_i \ge 1.2$ かつ $P_i \ge 0.02$ （一発逆転の旨味があり、最低限2%以上の勝率が見込める馬）
* **危険な人気馬**: $O_i \le 5.0$ （単勝5倍以下）かつ $EV_i < 0.8$

この分類をもとに、システムは以下の2つの戦略で「的中率・勝率の最大化（トリガミ回避）」を主眼としたポートフォリオ（予想買い目リスト）を自動生成する。
※生成した買い目はすべて、各馬の勝率を掛け合わせた独自の「合成的中確率（$hit\_probability$）」順でソートされ出力される。

### 戦略A（超堅実・鉄板回収型）
* **絶対軸（本命）** から、上位の対抗馬に向けて極限まで少点数で推奨するローリスク戦略。
* **券種と条件**: 
  * **単勝**: 本命馬の単勝オッズが1.0倍を上回る（元返し未然防止）場合のみ推奨。
  * **ワイド**: 本命から勝率2位〜4位への流し。ただし、オッズが「1.2倍以上」となる手堅く確実な買い目のみを推奨ラインとする。

### 戦略B（本命流し・少点数プラス確定型）
* **絶対軸（本命）** を軸にしつつ、確実なヒモ（勝率2〜6位）へ流して合成確率の最大化を狙う戦略。過剰な大穴狙いは行わない。
* **券種と条件**:
  * **馬連**: 本命から勝率2位〜6位への流し。点数を絞るためオッズ「2.5倍以上」を最低ラインとする。
  * **3連複**: 本命を1頭軸とし、相手（勝率2〜6位）へ流すフォーメーション。合成オッズの低下を防ぐため、オッズ「5.0倍以上」の組み合わせのみを拾い上げる。